...
一個使用 **Flutter** 和 **Provider** 狀態管理構建的現代化 Todo List 應用程式。

[功能特性](#功能特性) • [技術架構](#技術架構) • [快速開始](#快速開始) • [Provider 詳解](#provider-詳解) • [最佳實踐](#最佳實踐)
...
- 🗑️ **刪除待辦事項** - 滑動或點擊刪除按鈕
- 🔍 **篩選顯示** - 全部/活動中/已完成三種視圖
- 📊 **統計資訊** - 即時顯示總數、活動中、已完成數量
- 💾 **數據持久化** - 使用 SharedPreferences 保存數據
...
│   │   └── todo.dart               # Todo 數據模型
│   ├── providers/
│   │   └── todo_provider.dart      # TodoProvider (ChangeNotifier)
│   ├── screens/
